...
class Solution:
    def numberOfGoodSubsets(self, nums):
        ...
        twos = {6, 10, 14, 22, 26, 15, 21}
        counts = Counter(nums)

        r = {pq: 0 for pq in twos}
        for p in primes:
            for q in primes:
                if p > q and p*q < 30:
                    r[p*q] = Fraction(counts[p*q], (1+counts[p])*(1+counts[q]))

        ans = 1+sum(r.values())
        ans += ((r[22]+r[26])*(r[15]+r[21])+r[10]*r[21]+r[14]*r[15])
        ans += Fraction(counts[30], (1+counts[2])*(1+counts[3])*(1+counts[5]))
        for p in primes:
            ans *= (1+counts[p])
        ans -= 1

        return (ans*pow(2,counts[1])) % mod
